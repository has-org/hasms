		...
		>
			...
			<Stack>
				<List>
					...
					{items?.map((item: any) => {
						return (
							<Box key={`${item.product_code}-${item.color?.name}-${item.size?.name}`}>
								<ListItem sx={{ p: 0, m: 0 }}>
									<Stack
									...
									>
										<Stack direction='row' alignItems='center' spacing={1}>
											...
											<Stack>
												...
												<Stack direction='row' alignItems='center' spacing={1}>
													<CartTypography variant='body2'>{item.product_name}</CartTypography>
													<CartTypography variant='body2'>{item.color?.name}</CartTypography>
													<CartTypography variant='body2'>{item.size?.name}</CartTypography>
													<CartTypography variant='body2'>x{item.quantity}</CartTypography>
												</Stack>

												<Typography variant='body2'>{item.product_price} KM</Typography>
											</Stack>
										</Stack>
